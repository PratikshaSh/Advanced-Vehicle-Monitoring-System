...
class YOLO_Plate(object):
    _defaults = {
        "model_path": "C:/Users/TusharGoel/Desktop/Vehicle Object Tracking/Data/Model_Weights/trained_weights_final.h5",
        "anchors_path": "C:/Users/TusharGoel/Desktop/Vehicle Object Tracking/Data/Model_Weights/yolo_anchors.txt",
        "classes_path": "C:/Users/TusharGoel/Desktop/Vehicle Object Tracking/Data/Model_Weights/data_classes.txt",
        "score": 0.3,
        "iou": 0.45,
    ...
    def detect_image(self, image, show_stats=True):
        ...
        for a,c in reversed(list(enumerate(out_classes))):
            predicted_class = self.class_names[c]
            score = out_scores[a]
            if predicted_class == 'plate':
...
cv2.setMouseCallback("result", mouseEventHandler)
...
def detect_video(yolo, video_path, output_path=""):
    ...
    '''if len(mousePoints)==2:
        point_x_1 = mousePoints[0][0]
        point_y_1 = mousePoints[0][1]
        point_x_2 = mousePoints[1][0]
        point_y_2 = mousePoints[1][1]'''
# create a named window in OpenCv and attach the mouse event handler to it.
    ...
    while vid.isOpened():
        ...
        '''def line_definer(event,x,y,flags,param):
        ...
        cv2.setMouseCallback('result',line_definer)'''
            

        '''cv2.line(frame,(point_x_1,point_y_1),(point_x_2,point_y_2),(0,0,255),2)'''
        frame_no = frame_no+1
        if not return_value:
            break
        ...
        if len(out_pred) != 0:
            for i in range(len(out_pred)):
                # if int((out_pred[i][1]+out_pred[i][3])/2)>=int((int(point_y_1)+int(point_y_2))/2):
                    ...
                    # cv2.line(result,(point_x_1,point_y_1),(point_x_2,point_y_2),(0,255,0),2)
                    
                   
                    if out_pred[i][5] >0.3:
                            roi = frame[out_pred[i][1]:out_pred[i][3],out_pred[i][0]:out_pred[i][2]]
                            cv2.imwrite('C:/Users/Tushar Goel/Desktop/objects/{} {}.jpg'.format(frame_no,i),roi)
    ...
    out.release()
    # yolo.close_session()
def detect_image_Plate(self, image, show_stats=True):
        start = timer()
